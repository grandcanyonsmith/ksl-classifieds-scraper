...
def get_keywords():
    url = "https://ksl-classifieds.herokuapp.com/api/keywords"
    resp = requests.get(url)
    keywords = []
    if resp.status_code == 200:
        return resp.get("keywords")
...
while True:
    ...
    def get_urls(driver, search_url, website_url):
        ...
        try:
            while n > 0:
                ...
                container = soup.find(
                    'section', attrs={'class': 'search-page-results'})
            ...
            item_list = container.find_all(
                'a', attrs={'class': 'listing-item-link'})
    ...
    def get_container(driver, url):
        ...
        try:
            while n > 0:
                ...
                container = soup.find(
                    'article', attrs={'id': 'listingContainer'})
                if container is None:
                    driver.refresh()
                    n -= 1
                else:
    ...
    keywords = [keyword.get('item_name') for keyword in get_keywords()]
    ...
    driver = start_driver()
    driver.get(
        "https://www.ksl.com/myaccount/login?forward=https%3A%2F%2Fclassifieds.ksl.com%2F")
    time.sleep(2)
    ...
    for keyword in keywords:
        ...
        if urls:
            ...
            for url in urls:
                ...
                if container is not None:
                    title = container.find(
                        'h1', attrs={'class': 'listingDetails-title'})
                    title = title.text if title else ''
                    price = container.find(
                        'h2', attrs={'class': 'listingDetails-price'})
                    price = price.text if price else ''
                    views = container.find(
                        'span', attrs={'class': 'viewsDesktop-viewsNumber'})
                    views = views.text if views else ''
                    favorites = container.find(
                        'span', attrs={'class': 'viewsDesktop-favoritedNumber'})
                    favorites = favorites.text if favorites else ''
                    sellerName = container.find(
                        'span', attrs={'class': 'listingContactSeller-firstName-value'})
                    sellerName = sellerName.text if sellerName else ''
                    sellerPhone = container.find(
                        'span', attrs={'class': 'listingContactSeller-optionText'})
                    sellerPhone = sellerPhone.text if sellerPhone else ''
                    #print (views)
                    #print (favorites)

                    try:
                        viewAllLink = driver.find_element_by_class_name(
                            'photoViewer-viewAllLink')
                        driver.execute_script(
                            "arguments[0].click();", viewAllLink)
                    ...
                    # print (new_soup)
                    div_tags = new_soup.find('div', attrs={'class': 'photoViewer-imagesContainer'}).find_all(
                        'div', attrs={'class': 'carousel-carouselItem'})
                    #print (len(div_tags))
                    #print (div_tags)
                    ...
                    subset = (title, price, views, favorites,
                              sellerName, sellerPhone, url, img_final_str)
                    payload = {"title": title, "price": price, "views": views, "favorite": favorites,
                               'sellerName': sellerName, "sellerPhone": sellerPhone, 'url': url, 'img_final_str': img_final_str}

                    json_data = {"title": title, "price": price, "views": views, "favorites": favorites,
                                 'seller_name': sellerName, "seller_phone": sellerPhone, 'url': url, 'image_url': img_final_str}

                    print("Saving product to database")
                    # save using api
                    resp = requests.post(
                        "https://ksl-classifieds.herokuapp.com/api/add-product", json=json_data)
                    print(resp.status_code)
                    # product = Product(title=title, price=price, views=views, favorites=favorites,
                    #                   seller_name=sellerName, seller_phone=sellerPhone, url=url, image_url=img_final_str)
                    # product.save()
                    print("Saved to db...")
                    r = requests.post(
                        'https://hooks.zapier.com/hooks/catch/1739571/oq5s6rr/', data=payload)
                    print("Done:  ", subset)
            ...
            with open('{}.csv'.format(keyword), 'w', newline='', encoding='utf8') as f:
                writer = csv.writer(f)
                writer.writerow(['title', 'price', 'views', 'favorites',
                                 'sellerName', 'sellerPhone', 'url', 'Image Names'])
                writer.writerows(data)
        else:
            print('{} is taking too long to load. Proceeding to next keyword!'.format(
                search_url))
            ...
            if(j["favorites"] != 0):
                try:
                    ...
                    if(j["views"]/j["favorites"] > ratio_views_favorites and item_price < price_mean and item_price > minimum and item_price < maximum):
                        ...
                        text = "Hey, " + j["sellerName"] + " Do you still have the " + \
                            j["url"] + " ? Could you do " + str(offer) + "?"
                        print(text)
                        print("Price is Good")
                        print()
